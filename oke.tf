...
resource "oci_containerengine_cluster" "cluster" {
  ...
  dynamic "endpoint_config" {
    ...
    content {
      ...
      subnet_id            = var.use_existing_vcn ? var.api_subnet_id : oci_core_subnet.api_subnet[0].id
    }
  }

  options {
    service_lb_subnet_ids = [var.use_existing_vcn ? var.loadbalancer_subnet_id : oci_core_subnet.loadbalancer_subnet[0].id]
...
resource "oci_containerengine_node_pool" "pools" {
  ...
  node_config_details {
    placement_configs {
      ...
      subnet_id           = var.use_existing_vcn ? var.nodes_subnet_id : oci_core_subnet.nodes_subnet[0].id
    }
